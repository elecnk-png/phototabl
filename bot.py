...
async def save_entry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(f"Ошибка в save_entry: {e}")
...
async def save_entry_from_callback(query, context):
    ...
    try:
        ...
        logger.error(f"Ошибка в save_entry_from_callback: {e}")
...
async def _save_entry_data(user, message_or_query, context):
    ...
    try:
        ...
        logger.error(f"Ошибка в _save_entry_data: {e}")
        raise
...
async def show_entries(query, context):
    ...
    try:
        ...
        logger.error(f"Ошибка в show_entries: {e}")
...
def main():
    ...
    try:
        ...
        # Используем polling без дополнительных параметров
